		...
		UserSpecificEmail.findOne({ _userId: req.user._id }, (err, userEmails) => {
			if (userEmails) {
				...
				res.render('landing_page', {
					emailList: [{ to: 'name@example.com', subject: 'your subject here', time: 'YYYY-MM-DD HH:MM' }],
				});
			}
		});
	} else {
...
app.post('/send-mail-now', (req, res) => {
	...
	var timeStamp = [];
	if(parseInt(timeArray[1])-30 >= 0){
		timeStamp.push((parseInt(timeArray[0])-5).toString());
		timeStamp.push((parseInt(timeArray[1])-30).toString());
	} else {
		timeStamp.push((parseInt(timeArray[0])-6).toString());
		timeStamp.push((parseInt(timeArray[1])+30).toString());
	}
	const date = new Date(dateArray[0], dateArray[1] - 1, dateArray[2], timeStamp[0], timeStamp[1], 0);
